...
/* generated from python unicodedata 16.0: Mn + Me, plus ZWJ/ZWNJ and the variation
 * selectors, which sanitize keeps */
static const struct range zerow[] = {
	...
	{0x1A65,0x1A6C},{0x1A73,0x1A7C},{0x1A7F,0x1A7F},{0x1AB0,0x1ADD},{0x1AE0,0x1AEB},{0x1B00,0x1B03},
	...
	{0x1CF8,0x1CF9},{0x1DC0,0x1DFF},{0x200C,0x200D},{0x20D0,0x20F0},{0x2CEF,0x2CF1},
...
static const struct range wide[] = {
	{0x1100,0x115E},{0x231A,0x231B},{0x2329,0x232A},{0x23E9,0x23EC},{0x23F0,0x23F0},
	...
	{0x16FF0,0x16FF6},{0x17000,0x187FF},{0x18800,0x18CD5},{0x18CFF,0x18D1E},{0x18D80,0x18DF2},{0x1AFF0,0x1AFF3},
	...
	{0x1FA80,0x1FA8A},{0x1FA8E,0x1FAC6},{0x1FAC8,0x1FAC8},{0x1FACD,0x1FADC},{0x1FADF,0x1FAEA},{0x1FAEF,0x1FAF8},
	{0x20000,0x2FFFD},{0x30000,0x3FFFD},
};

/* invisible or direction-flipping: dropped outright, not measured. keeping them
 * would let a sender hide or reorder what a reader sees. covers every Cf except the
 * ZWJ/ZWNJ zerow keeps, every default-ignorable, and the U+2028/2029 line breaks */
static const struct range drop[] = {
	{0xAD,0xAD},{0x600,0x605},{0x61C,0x61C},{0x6DD,0x6DD},{0x70F,0x70F},
	{0x890,0x891},{0x8E2,0x8E2},{0x115F,0x11FF},{0x180E,0x180E},
	{0x200B,0x200B},{0x200E,0x200F},{0x2028,0x2029},{0x202A,0x202E},{0x2060,0x206F},
	{0x3164,0x3164},{0xD7B0,0xD7C6},{0xD7CB,0xD7FB},{0xFEFF,0xFEFF},{0xFFA0,0xFFA0},
	{0xFFF0,0xFFFB},
	{0x110BD,0x110BD},{0x110CD,0x110CD},{0x13430,0x1343F},{0x1BCA0,0x1BCA3},
	{0x1D173,0x1D17A},{0xE0000,0xE00FF},{0xE01F0,0xE0FFF}
};

static int inset(const struct range *r, size_t n, uint32_t cp)
{
	size_t lo = 0, hi = n;
	while (lo < hi) {
		size_t m = lo + (hi - lo) / 2;
...
#define IN(t, cp) inset(t, sizeof t / sizeof *t, cp)

size_t text_step(const void *sv, size_t n, uint32_t *cp)
{
	const uint8_t *s = sv;
	uint8_t b = s[0];
	uint32_t v, lo;
	...
	*cp = 0xFFFD;
	...
	if (b >= 0xC2 && b <= 0xDF)      { need = 2; v = b & 0x1Fu; lo = 0x80; }
	else if (b >= 0xE0 && b <= 0xEF) { need = 3; v = b & 0x0Fu; lo = 0x800; }
	else if (b >= 0xF0 && b <= 0xF4) { need = 4; v = b & 0x07u; lo = 0x10000; }
	else return 1;                  /* continuation byte, or a lead that is always overlong */

	for (i = 1; i < need && i < n; i++) {
		if ((s[i] & 0xC0) != 0x80) break;
		v = v << 6 | (s[i] & 0x3Fu);
	}
	if (i < need) return i;         /* cut short or a bad continuation: consume what was valid */
...
{
	if (IN(zerow, cp) || IN(drop, cp)) return 0;
	return IN(wide, cp) ? 2 : 1;
}

int text_width(const void *sv, size_t n)
{
	const uint8_t *s = sv;
	int w = 0;
	for (size_t i = 0; i < n; ) {
		uint32_t cp;
		i += text_step(s + i, n - i, &cp);
		if (cp >= 0x20 && cp != 0x7F) w += text_cp_width(cp);
	}
	return w;
}

static size_t emit(uint8_t *out, size_t cap, size_t at, uint32_t cp)
{
	uint8_t b[4];
	size_t n;
	...
	if (at + n <= cap)              /* all or nothing: a split sequence is not utf-8 */
		for (size_t i = 0; i < n; i++) out[at + i] = b[i];
	return n;
}

size_t text_sanitize(void *outv, size_t cap, const void *inv, size_t n)
{
	const uint8_t *in = inv;
	uint8_t *out = outv;
	size_t i = 0, w = 0;
	int marks = 0;
	...
	while (i < n) {
		uint32_t cp;
		i += text_step(in + i, n - i, &cp);
		if (cp == '\t') cp = ' ';
		/* esc goes as a control, so what followed it stays as visible text.
		 * no sequence parser: a terminal never sees the introducer */
		if (cp != '\n' && (cp < 0x20 || (cp >= 0x7F && cp <= 0x9F) || IN(drop, cp)))
			continue;
		if (text_cp_width(cp) == 0) {
			if (++marks > TEXT_MAX_MARKS) continue;
		} else marks = 0;
		w += emit(out, cap, w, cp);
	}
	return w;
